...
describe('GetSwiftCodeDetailsQueryHandler', () => {
  ...
  it('should return null when swift code does not exist', async () => {
    ...
    const result = await handler.execute(
      new GetSwiftCodeDetailsQuery('INVALIDXXX'),
    );

    expect(result).toBeNull();
    expect(mockRepository.findSwiftCodeWithBankAndCountry).toHaveBeenCalledWith(
      'INVALIDXXX',
    );
  });
  ...
  it('should return swift code details with branches when code is headquarters', async () => {
    ...
    mockRepository.findSwiftCodeWithBankAndCountry.mockResolvedValue(
      headquarterData,
    );
    mockRepository.findBranchesByHeadquarter.mockResolvedValue(branchData);

    const result = await handler.execute(
      new GetSwiftCodeDetailsQuery('TESTXXXX'),
    );
    ...
    expect(mockRepository.findBranchesByHeadquarter).toHaveBeenCalledWith(
      'TESTXXXX',
    );
  });
  ...
  it('should return swift code details without branches when code is not headquarters', async () => {
    ...
    const result = await handler.execute(
      new GetSwiftCodeDetailsQuery('TEST1111'),
    );
