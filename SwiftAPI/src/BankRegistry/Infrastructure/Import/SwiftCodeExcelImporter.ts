...
export class SwiftCodeExcelImporter {
    ...
    private async processRows(rows: SwiftCodeExcelRow[]): Promise<void> {
        for (const [index, row] of rows.entries()) {
            try {
                ...
                this.logger.error(`Failed to process row ${index + 1}: ${error.message}`);
                if (!error.message.includes('duplicate key value')) {
                    throw error;
                }
                this.logger.warn(`Skipping duplicate SWIFT code entry and continuing with import`);
            }
        }
    }

    private async processRow(row: SwiftCodeExcelRow, manager: EntityManager): Promise<void> {
    ...
    private async findOrCreateCountry(row: SwiftCodeExcelRow, manager: EntityManager): Promise<CountryEntity> {
    ...
    private async findOrCreateAddress(
        ...
        if (!address) {
            ...
            this.logger.debug(`Created new address: ${addressValue}, ${townNameValue}`);
        }

        return address;
    }

    private async findOrCreateBank(row: SwiftCodeExcelRow, manager: EntityManager): Promise<BankEntity> {
