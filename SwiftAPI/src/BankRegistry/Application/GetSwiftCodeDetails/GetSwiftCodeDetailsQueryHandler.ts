    ...
    implements IQueryHandler<GetSwiftCodeDetailsQuery>
{
    ...
    async execute(
        ...
        const swiftCodeData = await this.swiftCodeRepository.findSwiftCodeWithBankAndCountry(swiftCode);
        if (!swiftCodeData) {
            return null;
        }
        if (swiftCodeData.isHeadquarter) {
            const branches = await this.swiftCodeRepository.findBranchesByHeadquarter(swiftCode);
            return {
